...
void pushToPlace(int n, type Type, void *a, list_t *list){
    if(n>list->cnt+1||n<0) printf("list to short\n");
    else if((n==0)||(n==list->cnt)) (n==0)? pushBack(Type, a, list) : pushFront(Type, a, list);
    else{
        node_t *tmp, *new;
